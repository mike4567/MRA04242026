...
export function buildERDDAPMapUrl(
    ...
    const bboxDegrees = options?.bboxDegrees ?? 1.5;  // 1.5 degrees = ~165km from center (zoomed 25% from 2°)
    ...
    // Use a date 2 days ago to ensure data is available
    // MUR SST data has ~24-48 hour delay from present
    const safeDate = new Date();
    safeDate.setDate(safeDate.getDate() - 2);
    const safeTimeStr = safeDate.toISOString().split('T')[0] + 'T09:00:00Z';
    ...
    const params = new URLSearchParams({
        ...
        FORMAT: 'image/png',
        TIME: safeTimeStr,  // Use date 2 days ago to ensure data availability
 ...
 * Returns the actual SST value at the specified location using the most recent available data.
 * 
 * Note: MUR SST data has a ~24-48 hour delay, so we use "last" to get the most recent
 * available data rather than the exact incident time which may not exist yet.
 * 
 * @param lat - Latitude of incident location
 * @param lng - Longitude of incident location  
 * @param timestamp - Time of incident (stored for reference, but we fetch "last" available data)
...
export async function fetchERDDAPData(
    ...
    try {
        // Build GridDAP JSON URL using "last" for the most recent available data
        // ERDDAP GridDAP syntax: variable[(time)][(latitude)][(longitude)]
        // MUR SST data has ~24-48hr delay, so "last" gets most recent available
